...
# Metilinimo matrica ir anotacijos
meth <- as.matrix(obj)
meta <- as.data.frame(colanns(obj))

annot <- data.frame(
  sample_id = colnames(meth),
  cell_type = as.character(meta[["celltype"]]),
  diagnosis = as.character(meta[["diagnosis"]]),
  stringsAsFactors = FALSE
)

# Labiausiai varijuojančios CpG
n_top <- 10000

row_var <- apply(meth, 1, var, na.rm = TRUE)
row_var[is.na(row_var)] <- -Inf

top_idx <- order(row_var, decreasing = TRUE)[1:n_top]
meth_top <- meth[top_idx, ]

cat("Pradinės matricos dydis:", dim(meth), "\n")
cat("Atrinktų CpG matricos dydis:", dim(meth_top), "\n")

# Pearson koreliacija tarp mėginių
cor_mat <- cor(meth_top, method = "pearson", use = "pairwise.complete.obs")
...
summarize_pairs <- function(df, label) {
  ...
  cat("Vidutinė koreliacija vienodose porose:", round(mean(same_vals, na.rm = TRUE), 4), "\n")
  cat("Vidutinė koreliacija skirtingose porose:", round(mean(diff_vals, na.rm = TRUE), 4), "\n")
  cat("Mediana vienodose porose:", round(median(same_vals, na.rm = TRUE), 4), "\n")
  cat("Mediana skirtingose porose:", round(median(diff_vals, na.rm = TRUE), 4), "\n")
...
# PCA
pca <- prcomp(t(meth_top), center = TRUE, scale. = FALSE)
